...
' Exports all modules as raw text chunks and opens the folder automatically
Public Sub VBASync_ExportAllToChunks(ByVal folderPath As String)
    ...
    ' 1. Count exportable components first for the header denominator
    totalCount = 0
    For Each comp In ThisWorkbook.VBProject.VBComponents
        If IsExportable(comp) Then totalCount = totalCount + 1
    Next comp
    ...
    For Each comp In ThisWorkbook.VBProject.VBComponents
        If IsExportable(comp) Then
            ...
            ' Sanitize filename using legacy logic
            finalPath = folderPath & VBASync_SanitizeName(comp.Name) & ".txt"
            
            ' Format content with your strict header: FILE X/TOTAL: Name
            Dim chunkContent As String
            chunkContent = "FILE " & currentCount & "/" & totalCount & ": " & comp.Name & vbCrLf & _
                           String(30, "-") & vbCrLf & _
                           StripVBHeaders(comp)
            
            WriteFile finalPath, chunkContent
        End If
    Next comp
    ...
    MsgBox "Exported " & totalCount & " chunks to: " & vbCrLf & folderPath, vbInformation
End Sub
